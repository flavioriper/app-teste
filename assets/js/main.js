$(document).ready(function () {
    ...
    $('#vista-integration').submit(function(ev) {
        ev.preventDefault();
        const url = $(this).attr('action');
        const host = $('#vista-host').val();
        const token = $('#vista-token').val();
        showLoader("body");
        $.post(url, { host, token }).done(function(data){
            showLoader("body", false);
            if (data == '200') {
                showModal('Atualização autorizada', 'Os dados foram atualizados.');
            } else {
                showModal('Atualização não autorizada', 'Os dados não foram atualizados, entre em contato com o administrador.');
            }
        });
    });
