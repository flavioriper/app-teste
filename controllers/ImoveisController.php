...
class ImoveisController extends BaseController { 
    ...
    public $route = 'imoveis';

    public static function status($imovel) {

        if(self::authorization($imovel, 'imoveis')) {
    ...
    public static function list() {
        ...
        $data['vista_status'] = vista::getStatus();
        self::render($data, 'imoveis/list');
    ...
    public static function form() {
         ...
         *-----------------------------------------------------*/
        if(!isset($_GET['id'])) {
            if(!base::publishVerify( base::publishData($user, 'imovel') )) {
                Flight::redirect('/imoveis');
                exit();
            }
        }
        ...
        if( !isset($_GET['id']) ) {
            ...
            header('Location:' . str_replace('yuppins/app/', 'app/', get_site_url() . $_SERVER['REQUEST_URI'] . (empty($_GET) ? '?id=' . $id : '&id=' . $id)));
            exit;
        }
         ...
         *-----------------------------------------------------*/
        elseif( get_post_status ( $_GET['id'] ) &&  $user != get_post( $_GET['id'] )->post_author ) {
         ...
         *-----------------------------------------------------*/
            ...
            Flight::notFound('Imovel não encontrado');
        }
         ...
         *-----------------------------------------------------*/
        self::render($data, 'imoveis/form');
    }

    public static function create($imovel) {
        // echo '<pre>';print_r($imovel);exit;
    ...
    public static function image_upload($imovel) {
        ...
        $i = 1;
        // var_dump($wordpress_upload_dir);exit;
